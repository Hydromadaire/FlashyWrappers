CALL config.bat
CALL %FLEX_PATH%acompc -swf-version=18 -target-player=11.5 -define+=CONFIG::AIR,true -define+=CONFIG::FLASCC,false -define+=CONFIG::CODECS,"'MP4'" -source-path="%cd%"  -include-sources "com/rainbowcreatures/" -output com.rainbowcreatures.FWVideoEncoder.swc
mkdir tmp
unzip -o -q com.rainbowcreatures.FWVideoEncoder.swc -d tmp
copy Windows\extension.xml tmp\*.* /Y
copy Windows\MingW\mp4\nativeExtension.dll tmp\*.* /Y
