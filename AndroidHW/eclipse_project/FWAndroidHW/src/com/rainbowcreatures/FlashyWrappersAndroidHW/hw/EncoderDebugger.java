...
package com.rainbowcreatures.FlashyWrappersAndroidHW.hw;

import java.io.PrintWriter;
import java.io.StringWriter;
import java.nio.ByteBuffer;

import com.rainbowcreatures.FlashyWrappersAndroidHW.hw.CodecManager.Codec;
...
public class EncoderDebugger {
	...
	private void debug() {
		...
		Log.d(TAG, ">>>> Number of encoders: " + encoders.length + " <<<<");
		
		// Tries available encoders
		for (int i=0;i<encoders.length;i++) {
			Log.d(TAG, ">>>> Number of encoder '" + encoders[i].name + "' formats: " + encoders[i].formats.length + " <<<<");
...
//        mediaFormat.setInteger(MediaFormat.KEY_MAX_INPUT_SIZE, 0); // fix for Samsung deadbeef CRASH
	...
	private long encode() {
		...
		while (elapsed<1000) {
			...
			Log.d(TAG, ">>>> encInputIndex " + encInputIndex + " <<<<");
			if (encInputIndex>=0) {
